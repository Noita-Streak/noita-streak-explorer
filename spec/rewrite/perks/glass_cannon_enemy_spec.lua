...
local test_items = {
    ...
    { title = "タイプダメージが0.2 * 3.0に置換されていること", input = { "v = v * 3.0" }, output = { "v = v * 0.2 * 3.0" } },
    { title = "爆破設定が0.2 * 4.0に、nilの場合に読み込まない形に置換されていること", input = { "v = v * 4.0" }, output = { "if (v ~= nil) then v = v * 0.2 * 4.0" } },
    { title = "endが追加されていること", input = { "ComponentObjectSetValue( comp, \"config_explosion\", b, tostring(v) )" }, output = { "ComponentObjectSetValue( comp, \"config_explosion\", b, tostring(v) ) end" } }
}
